    ...
    public void injectedRandomDisplayTick(BlockState state, World world, BlockPos pos, Random random, CallbackInfo ci) {
        if (Configs.Particles.PARTICLE_GLOW_BERRY_SHINE.getBooleanValue() && (state.get(CaveVinesBodyBlock.BERRIES) || state.get(CaveVinesHeadBlock.BERRIES))&& random.nextInt(Configs.ParticleRarities.GLOW_BERRY_SHINE_PARTICLE_RARITY.getIntegerValue()) == 0) {
            ...
            if (direction != Direction.UP && direction != Direction.DOWN) {
                double d = (double)pos.getX() + 0.5 + (direction.getOffsetX() == 0 ? 0.5 - random.nextDouble() : (double)direction.getOffsetX() * 0.1);
                double e = (double)pos.getY() + 0.5 + (direction.getOffsetY() == 0 ? 0.5 - random.nextDouble() : (double)direction.getOffsetY() * 0.3);
                double f = (double)pos.getZ() + 0.5 + (direction.getOffsetZ() == 0 ? 0.5 - random.nextDouble() : (double)direction.getOffsetZ() * 0.1);
                world.addParticle(LittleDetailsParticleTypes.GLOW_BERRY_SHINE, d, e, f, 0.0, 0.0, 0.0);
            }
        }
    }
